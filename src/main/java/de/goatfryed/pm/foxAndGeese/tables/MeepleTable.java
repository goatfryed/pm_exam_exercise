...
import de.goatfryed.pm.foxAndGeese.Meeple;

import de.goatfryed.pm.foxAndGeese.Player;

import de.goatfryed.pm.foxAndGeese.Field;
...
public class MeepleTable 
{

   public MeepleTable(Meeple... start)
   {
      this.setColumnName("Meeple");
      columnMap.put(this.getColumnName(), 0);
      for (Meeple current : start)
      {
   ...
   public MeepleTable setTable(ArrayList<ArrayList<Object>> value)
   {
      this.table = value;
      return this;
   }
   ...
   public MeepleTable setColumnName(String value)
   {
   ...
   public MeepleTable setColumnMap(LinkedHashMap<String, Integer> value)
   {
   ...
   public PlayerTable expandPlayer(String... rowName)
   {
      ...
      {
         Meeple start = (Meeple) row.get(columnMap.get(this.getColumnName()));
   ...
   public MeepleTable hasPlayer(PlayerTable rowName)
   {
      ArrayList<ArrayList<Object> > oldTable = (ArrayList<ArrayList<Object> >) this.table.clone();
      this.table.clear();
      for (ArrayList<Object> row : oldTable)
   ...
   public FieldTable expandField(String... rowName)
   {
      ...
      for (ArrayList<Object> row : oldTable)
      {
         Meeple start = (Meeple) row.get(columnMap.get(this.getColumnName()));
         ArrayList<Object> newRow = (ArrayList<Object>) row.clone();
         newRow.add(start.getField());
         this.table.add(newRow);
      }
      return result;
   }

   public MeepleTable hasField(FieldTable rowName)
   {
      ArrayList<ArrayList<Object> > oldTable = (ArrayList<ArrayList<Object> >) this.table.clone();
      this.table.clear();
      for (ArrayList<Object> row : oldTable)
   ...
   public MeepleTable selectColumns(String... columnNames)
   {
   ...
   public MeepleTable dropColumns(String... columnNames)
   {
      LinkedHashMap<String, Integer> oldColumnMap = (LinkedHashMap<String, Integer>) this.columnMap.clone();
      this.columnMap.clear();
   ...
   public MeepleTable filter(Predicate< Meeple > predicate)
   {
      ArrayList<ArrayList<Object> > oldTable = (ArrayList<ArrayList<Object> >) this.table.clone();
      this.table.clear();
      for (ArrayList<Object> row : oldTable)
   ...
   public MeepleTable filterRow(Predicate<LinkedHashMap<String,Object> > predicate)
   {
      ArrayList<ArrayList<Object> > oldTable = (ArrayList<ArrayList<Object> >) this.table.clone();
      this.table.clear();
      for (ArrayList<Object> row : oldTable)
   ...
   public LinkedHashSet< Meeple > toSet()
   {
      LinkedHashSet< Meeple > result = new LinkedHashSet<>();
      for (ArrayList row : this.table)
      {
         Meeple value = (Meeple) row.get(columnMap.get(columnName));
         result.add(value);
      }
      return result;
   }
